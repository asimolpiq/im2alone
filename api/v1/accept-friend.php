...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['friendID'])) {
        ...
        if (isset($headers['Authorization'])) {
            ...
            if ($result != null) {
                $myID = $result['id'];
                $searchResponse = acceptFriend($conn, $friendID, $myID);
                if ($searchResponse) {
                    echo json_encode(array("status" => "success", "data" => "Arkadaşlık kabul edildi."));
                } else {
                    echo json_encode(array("status" => "success", "data" => "Arkadaşlık edilemedi."));
                }
            } else {
