...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['userID'])) {
        ...
        if (isset($headers['Authorization'])) {
            ...
            if ($result != null) {
                $myID = $result['id'];
                $unblockResponse = unblockUser($conn, $myID, $targetID);
                if ($unblockResponse) {
                    echo json_encode(array("status" => "success", "data" => "Engel kaldırıldı."));
                } else {
                    echo json_encode(array("status" => "error", "data" => "Engel kaldırılamadı."));
                }
            } else {
